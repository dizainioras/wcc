...
long fetchURL(const string& url, const string& outputFile) {
	...
	if (curl) {
		...
		dbg("FU set curl perform.");
		res = curl_easy_perform(curl);
		

		if (res == CURLE_OK) {
			dbg("FU set curl getinfo.");
			curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE, &RCode);
			dbg("FU set curl cleanup.");
	...
	dbg("FU set curl global cleanup.");
	curl_global_cleanup();
...
void readFile3(const std::string& filePath, std::string& cleanedText) {
	...
	dbg("Script remove end.");
	dbg("Style remove start.");
	bool findStyle = true;
