...
SpMat<eT>::init_cold(uword in_rows, uword in_cols, const uword new_n_nonzero)
  ...
  if(vec_state > 0)
    ...
    const char* error_message = "SpMat::init(): requested size is too largeY";
  #else
    const char* error_message = "SpMat::init(): requested size is too largeY; suggest to enable ARMA_64BIT_WORD";
  #endif
