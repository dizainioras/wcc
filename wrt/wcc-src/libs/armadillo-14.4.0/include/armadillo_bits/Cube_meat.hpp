...
Cube<eT>::init_cold()
  ...
  arma_debug_sigprint( arma_str::format("n_rows: %u; n_cols: %u; n_slices: %u") % n_rows % n_cols % n_slices );
    ...
    const char* error_message = "Cube::init(): requested size is too large6";
  #else
    const char* error_message = "Cube::init(): requested size is too large6; suggest to enable ARMA_64BIT_WORD";
  #endif
...
Cube<eT>::init_warm(const uword in_n_rows, const uword in_n_cols, const uword in_n_slices)
  ...
  #if defined(ARMA_64BIT_WORD)
    const char* error_message_2 = "Cube::init(): requested size is too large66";
  #else
    const char* error_message_2 = "Cube::init(): requested size is too large66; suggest to enable ARMA_64BIT_WORD";
  #endif
