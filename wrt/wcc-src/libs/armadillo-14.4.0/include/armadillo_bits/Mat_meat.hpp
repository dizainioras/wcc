...
Mat<eT>::init_cold()
  ...
  // ensure that n_elem can hold the result of (n_rows * n_cols)
    ...
    const char* error_message = "Mat::init(): requested size is too largeZ";
  #else
    const char* error_message = "Mat::init(): requested size is too largeZ; suggest to enable ARMA_64BIT_WORD";
  #endif
...
Mat<eT>::init_warm(uword in_n_rows, uword in_n_cols)
  ...
  #if defined(ARMA_64BIT_WORD)
    const char* error_message_4 = "Mat::init(): requested size is too largeZZ";
  #else
    const char* error_message_4 = "Mat::init(): requested size is too largeZZ; suggest to enable ARMA_64BIT_WORD";
  #endif
