...
MapMat<eT>::init_cold()
  ...
  // ensure that n_elem can hold the result of (n_rows * n_cols)
    ...
    const char* error_message = "MapMat(): requested size is too largeA";
  #else
    const char* error_message = "MapMat(): requested size is too largeA; suggest to enable ARMA_64BIT_WORD";
  #endif
...
MapMat<eT>::init_warm(const uword in_n_rows, const uword in_n_cols)
  ...
  #if defined(ARMA_64BIT_WORD)
    const char* error_message = "MapMat(): requested size is too largeXX";
  #else
    const char* error_message = "MapMat(): requested size is too largeXX; suggest to enable ARMA_64BIT_WORD";
  #endif
