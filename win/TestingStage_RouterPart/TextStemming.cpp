// Steming.cpp : Defines the entry point for the application.
...
static void step2() {
    ...
    case 'e':
        ...
        break;
    case 'l':
        if (ends("\03" "bli")) { r("\03" "ble"); break; } /*-DEPARTURE-*/
...
void increase_s() {
    i_max += INC;
    char* new_s = new char[i_max + 1];
    for (int i = 0; i < i_max; i++) new_s[i] = s[i];
    delete[] s;
    s = new_s;
}
...
string  stemfile(const string& text) {
    ...
    while (stream >> word) {
        ...
        // char array
        //strcpy_s(s, word.c_str()); // Tik Microsoft versijai

        
        strcpy(s, word.c_str());

        // jei negalima strcpy, tada naudoti strncpy 
        //strncpy(s, word.c_str(), i);

        s[stem(s, 0, i - 1) + 1] = 0;
       
        stemText << s << " ";

       //strcpy_s(s, ""); // Tik Microsoft versijai
